...
switch ($data['actions'] ?? '') {
    ...
    case 'user':
        ...
        try {
            ...
            $stmt = $pdo->prepare("SELECT SUM(price_product) as sum_price,COUNT(username) as count_invoice FROM invoice WHERE name_product != :mp1 AND  id_user = :user_id AND Status != 'Unpaid'");
